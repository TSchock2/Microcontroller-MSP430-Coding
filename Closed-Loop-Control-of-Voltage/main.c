...
// (main).c or .h
// This module contains files necessary for Main in Lab 8
// ----------------------------------------------------------------------------
/*******************************************************************************/
...
void main (void)
{
    ...
    while(1)
    {
        ...
        if(ISR_Flag == 1)                                           // Every 62.5 milliseconds for this Lab
        {
            ...
            if(RC_Voltage_Mode == TRUE)                             // RE - Calculate RC Voltage, which also calls RC_Feedback and update_RGB
            {
                RC_Voltage();
            }
            ...
            if(LED_Alternate == TRUE)                               // Test for 1/8 second timing
            {
                ...
                {
                    P3DIR |= 0xC0;
                    P2DIR |= 0x04;                                  // Set direction register PWM pins to outputs
                }
            }
        }
